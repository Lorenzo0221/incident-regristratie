<x-base-layout>
    <h1 class="mb-6 text-2xl font-bold">Gebruiker toevoegen</h1>
    ...
    <form method="POST" action="{{ route('users.store') }}" class="space-y-4 rounded bg-white p-6 shadow">
        @csrf
        ...
        <div>
            ...
            <select name="role" class="w-full rounded border-gray-300" required>
                <option value="gebruiker" @selected(old('role', 'gebruiker') === 'gebruiker')>Gebruiker</option>
        ...
        <div>
            <label class="mb-1 block text-sm font-medium">Bevestig wachtwoord</label>
